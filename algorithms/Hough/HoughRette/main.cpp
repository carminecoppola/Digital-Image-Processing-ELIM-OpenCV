...
void convertPolar(double rho, double theta, Point& p1, Point& p2){
    ...
    /**
     * I due punti pt1 e pt2 dovranno essere spostati lungo la retta (per questo moltiplichiamo per alpha)
     * ma in direzioni opposte dalla posizione iniziale (per questo in pt1 sommiamo mentre in pt2 sottraiamo).
    */
...
void houghRette(const Mat& src, Mat& dst ){
     ...
     * Questo corrisponde a considerare una possibile distanza ρ (che andrà da ρ a – ρ) 
     * (è la diagonale calcolata con hypot) e dei possibili angoli θ (180°), quindi la 
     * nostra quantizzazione avrà ρ unità e considererà θ angoli.
    */
   ...
   Mat votes = Mat::zeros(maxDist*2, 180, CV_8UC1);
     ...
     * Per votare i pixel di edge dovranno tener conto di tutte le possibili direzioni, che in 
     * questo caso sarebbero tutte quelle comprese in un angolo da -90° a 90°.
     ...
     * 
     * Nb: il motivo per cui x e y sono invertite rispetto alla formula originale è perché in 
     * realtà non siamo nel 1° quadrante ma nel 4°
     * Nb: al valore calcolato dalla formula, dobbiamo aggiungere maxDist, sarà la nostra 
     * costante da sottrarre nella conversione polare per trovare il giusto rho ad ogni iterazione. 
     * 
