 ...
 *  2. Dopo la fase di split, si applica il processo di merging a tutte le regioni adiacenti
 *     Ri e Rj, per cui Q(Ri U Rj) = Vero;
 *  3. Il processo termina quando non è più possibile effettuare unioni.
 * 
 * Solitamente si definisce una dimensione minima della regione oltre la quale non si 
 * effettua lo split. Per ragioni di efficienza, la fase di merge si può eseguire se 
 ...
 * 
 * Ho una vettore di puntatori a Tnode merged che rappresenta le regioni che devono essere 
 * unite (queste regioni saranno i figli).
 * 
 ...
 * 
 * Per determinare il predicato dovrò calcolare la deviazione standard stddev e la media 
 * della regione. In particolare lavoro sulla deviazione standard mentre userò la media
 * per assegnare un’etichetta alla regione finale.
 * 
...
/**
 * Split si tratta di una funzione ricorsiva: se entro nella funzione split 
 * significa che ho una nuova regione e quindi alloco un nuovo Tnode. 
 * R è la regione dell’immagine rappresentata dal nodo. 
 * Ovviamente, la prima volta (dunque nel main) R parte da (0,0) e l’altezza
 * e l’ampiezza dell’immagine sono il numero di righe e di colonne 
 * dell’immagine, dunque vado a prendere tutta l’immagine.
 * 
 * Poi mi calcolo media e deviazione standard(quanto i volori dei pixel distano
 * dalla media) all’interno della regione attraverso la funzione di OpenCV meanStdDev. 
 * Una volta calcolati, li vado ad impostare negli attributi corrispondenti di root.
 * 
 * Poi prima di iniziare la fase di splitting ricorsiva, dobbiamo controllare se la
 * regione > tsize, dunque se quella regione può essere ulteriormente splittata, e 
 * se la deviazione standard è maggiore di un determinato threshold. 
 * In tal caso, effettuo lo splitting. Avrò 4 regioni tutte che avranno la metà 
 * dell’ampiezza e dell’altezza dell’immagine originale.
 * 
 * Per ogni regione calcolata, per il nodo corrispondete dovrò richiamare ricorsivamente
 * la funzione split a cui passerò l’immagine originale e la regione calcolata. 
 ...
 * dunque se la regione considerata è troppo piccola oppure la deviazione standard è 
 * sotto la soglia (dunque ho una regione piuttosto uniforme come livelli di grigio).
 * 
 * Inoltre, visualizzo il contorno del rettangolo sull’immagine (rectangle(img,R,Scalar(0)),
 * dunque in output avremo il Quadtree.
 * 
 ...
 * 
 * Prima di analizzare la regione verifico se l’ampiezza è maggiore di tsize e se la deviazione standard 
 * è maggiore della threshold: 
 * in tal caso, significa che la regione è stata splittata, e quindi ho delle sottoregioni che potrei fondere.
 * 
 * Altrimenti, significa che la regione non è stata splittata, dunque inserisco all’interno dell’array merged
 * della regione l’intera regione stessa, in quanto deve essere considerata integralmente.
 * 
 * Come flag per indicare che tutta la regione deve essere unita imposto a true tutti i valori booleani 
 * dell’array MergedB.
 * 
 ...
 *      Per verificare se queste due regioni possono essere unite dobbiamo prendere in considerazione 
 *      la deviazione standard stddev della regione e verificare se entrambe sono minori di una certa
 *      threshold (N.B in questo caaso si utilizza anche la media).
 *      Se così fosse, inserisco ambo le regioni nell’array merged e imposto ambo i corrispettivi 
 *      booleani di mergedB a true.
 * 
 ...
 * 
 * Se l’array non contiene nulla significa che quella regione è stata splittata in 4, e dunque
 * richiamo segment sulle 4 sottoregioni.
 * 
 * Altrimenti, nell’array merged potrò avere 1 valore o >1 valori.
 * 
 * All’interno vado a metterci la media dei pixel contenuti all’interno della regione, o per 
 * motivi di efficienza faccio direttamente la “media delle medie”.
 * 
 * Poi assegno questa approssimazione del valore medio val in tutte le regioni che sono state 
 * inserite in tmp.
 * 
 *  - Se è 1, significa che ho inserito tutta la regione. In pratica si tratta di una foglia 
 *    del nostro quadtree. Non faccio nessun altra operazione.
 * 
 *  - Se è >1, significa che ho inserito più di una regione. 
 *    Andrò dunque a verificare quali di queste non sono state unite, cioè vado a vedere in 
 *    ogni posizione di mergedB
 *    (con cui mi segno proprio quali regioni ho unito) se il valore è false, 
 *    perché in tal caso significa che la regione non è stata segmentata e dunque richiama 
 *    ricorsivamente segment su quella regione.
*/
