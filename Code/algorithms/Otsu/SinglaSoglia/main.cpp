...
vector<float> normHistogram(const Mat&src){
    vector<float> hist(256, 0.0f);       //è il vettore che rappresenta l'istogramma, è composto da 256 elementi float inizializzati a 0
...
/**
 * Calcoliamo la media cumulativa m(k).
 * 
 * Per ogni soglia i, hist[i] è  moltiplicata per i (ovvero la soglia corrente).
 * Essendo anche in questo caso cumulative, la somma al passo i-simo sarà data
 * considerando la somma al passo precedente.
 * 
 ...
 * 
 * Consideriamo un vettore nel quale calcoliamo la varianza interclasse per ogni soglia i-esima.
 * 
...
/**
 * Otteniamo k*, la soglia ottimale per cui σ2B(k) è massima, 
 * calcolando la posizione del massimo del vettore sigma.
*/
