...
/**
 * filteredValue() calcola il valore filtrato per un punto specifico nell'immagine con una 
...
uchar filteredValue(Mat padd, Mat mask, int offx, int offy){

    float value = 0;

    for (int i = 0; i < mask.rows; i++)
        for (int j = 0; j < mask.cols; j++)
            value += mask.at<float>(i,j) * padd.at<uchar>( i+offx, j+offy );
